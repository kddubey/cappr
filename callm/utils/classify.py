...
def predict_proba(conditional_func):
    ...
    def wrapper(prompts, completions, model, prior=None, func=None, **kwargs
                ) -> npt.NDArray[np.float64]:
        func = _check_func(func)
        log_probs_completions = conditional_func(prompts, completions, model,
                                                 **kwargs)
...
def predict_proba_examples(conditional_examples_func):
    ...
    def wrapper(examples: Sequence[Example], model, func=None, **kwargs
                ) -> Union[list[list[float]], npt.NDArray[np.float64]]:
        func = _check_func(func)
        log_probs_completions = conditional_examples_func(examples, model,
                                                          **kwargs)
