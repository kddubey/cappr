...
def add_doc_after(docstring: str):
    '''
    Returns a decorator which concatenates `docstring` to the decorated
...
def wraps_but_keep_wrapper_return_ann(wrapped,
    ...
    ```
    would return, except that the `'return'` annotation from the wrapper
    function is used instead of that from `wrapped`.
    '''
