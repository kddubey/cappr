    ...
    def forward(
        ...
        if self._batch_idxs is None:
            ...
            if encodings_past["input_ids"].shape[0] == 1 and input_ids.shape[0] > 1:
                # Assume the past of input[i] is past[0] (the only past)
                ...
                # Assume the past of input[i] is past[i]
...
def cache(
    ...
    Usage with :func:`predict_proba`::
        ...
        from cappr.huggingface.classify_ import cache
...
def _last_nonpad_token_logits_correct(
    logits: torch.Tensor, attention_mask: torch.Tensor
):
    last_nonpad_token_idxs = (
        -1 - (attention_mask.flip(dims=[1]) == 1).max(dim=1).indices
    )
    return torch.stack(
        [logits[i, idx] for i, idx in enumerate(last_nonpad_token_idxs)]
    ).unsqueeze(dim=1)
...
def _blessed_helper(
    ...
    # Encode them all at once and then batch so that logits.shape[1] is identical.
    ...
    assert len(prompts_idxs) == num_batches
    # fmt: off
    ...
    # fmt: on
    # TODO: put this in the context manager?
    in_caching_context = isinstance(model, _ModelWithCache)
    start_of_prompt = " " if in_caching_context else ""
    ...
    with cache((model, tokenizer), prompts, logits_all=False) as cached_prompts:
        prompts_last_nonpad_token_logits = _last_nonpad_token_logits(
            cached_prompts[0]._cappr_past[1].logits,
            cached_prompts[0]._cappr_past[0]["attention_mask"],
        )
        if (
            _are_completions_constant
            and completions_encoding["input_ids"].shape[1] == 1
        ):
        ...
        offsets = cached_prompts[0]._cappr_past[0]["attention_mask"].sum(dim=1)
        completions_logits = []
        _batch_idxs = cached_prompts[0]._batch_idxs
        for batch_idx in range(num_batches):
            cached_prompts[0]._batch_idxs = prompts_idxs[batch_idx]
            out: CausalLMOutput = cached_prompts[0](
        ...
        cached_prompts[0]._batch_idxs = _batch_idxs

    completions_logits = torch.cat(completions_logits, dim=0)
    ...
    completions_logits = torch.cat(
        [prompts_last_nonpad_token_logits, completions_logits[:, :-1, :]], dim=1
    )
